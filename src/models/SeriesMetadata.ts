...
SeriesMetadataSchema.statics.getByDirOrFilename = async function(dirOrFilename): Promise<SeriesMetadataInterface | null> {
  const parsed = episodeParser(dirOrFilename);
  const seriesName = parsed && parsed.show ? parsed.show : null;
  if (seriesName) {
    return await this.findOne({ title: seriesName }).lean();
  }
};
